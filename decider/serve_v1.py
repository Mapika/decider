...
It differs from decider.serve in: torch.compile and FP8 on by default (DECIDER_COMPILE=1, DECIDER_FP8=1), graphs captured
for lengths up to 1,536 tokens with the rest captured or run eager on the request path, no request-size or queue limits, and
DECIDER_MAX_WAIT_MS accepted but unused (DECIDER_BATCH_WAIT_MS is the collection window).  Removed in 1.2.0.
