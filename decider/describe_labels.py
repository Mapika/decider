...
question only (no eval text is shown to the teacher).
   python -m decider.describe_labels data/label_desc.json"""
import argparse, collections, json, random, re, time, torch
from . import data as D
from . import data2, data3  # noqa

SKIP_TASKS = {"mario", "synth", "games", "hh_rlhf", "shp", "ultrafeedback_pref", "reward_bench", "arena_pref", "helpsteer3_pref"}
...
def fixed_label_sets(train, evals, min_n=50):
    ...
    for k, c in sets.items():
        ...
        if n / sum(c.values()) > 0.98 and sum(c.values()) >= min_n and k[0] not in SKIP_TASKS:
...
def main():
    ...
    sets = fixed_label_sets(train, evals)
